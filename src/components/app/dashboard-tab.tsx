...
import { BarChart, Line, LineChart, CartesianGrid, XAxis, YAxis, Tooltip, Legend, ResponsiveContainer } from 'recharts';
  ...
  if (!isCalculated || !calculatedData) {
    return (
      <div className="flex items-center justify-center h-full text-muted-foreground">
        Preencha o formulário para ver o painel.
      </div>
    );
  }
      ...
      <Card>
        ...
        <CardContent>
          <ChartContainer config={chartConfig} className="h-[250px] w-full">
            <LineChart data={calculatedData.brixEvolution} margin={{ top: 5, right: 20, left: -10, bottom: 5 }}>
              <CartesianGrid strokeDasharray="3 3" />
              <XAxis dataKey="name" />
              <YAxis />
              <ChartTooltip content={<ChartTooltipContent />} />
              <Legend />
              <Line type="monotone" dataKey="brix" stroke="hsl(var(--primary))" name="Brix (%)" />
            </LineChart>
          </ChartContainer>
        </CardContent>
      </Card>
        ...
        <CardContent>
          <ChartContainer config={chartConfig} className="h-[250px] w-full">
            <BarChart data={calculatedData.effectEfficiency} margin={{ top: 5, right: 20, left: -10, bottom: 5 }}>
              ...
              <ChartTooltip content={<ChartTooltipContent />} />
              <Legend />
              <BarChart dataKey="efficiency" fill="hsl(var(--primary))" name="Eficiência (%)" />
            </BarChart>
          </ChartContainer>
        </CardContent>
      </Card>
        ...
        <CardContent>
          <ChartContainer config={chartConfig} className="h-[250px] w-full">
            <BarChart data={calculatedData.vaporGeneration} margin={{ top: 5, right: 20, left: -10, bottom: 5 }}>
              ...
              <ChartTooltip content={<ChartTooltipContent />} />
              <Legend />
              <BarChart dataKey="generation" fill="hsl(var(--primary))" name="Vapor (t/h)" />
            </BarChart>
          </ChartContainer>
        </CardContent>
      </Card>
